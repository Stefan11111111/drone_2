...
toDdsDuration(const std::chrono::milliseconds timeout)
{
    ...
    {
        throw std::invalid_argument{"An ExplosionEvent data wait timeout must not be negative"};
    }
...
{
    throw std::runtime_error{"Fast DDS could not take ExplosionEvent sample; return code " +
                             std::to_string(returnCode)};
}

} // namespace

ExplosionEventReader::ExplosionEventReader(eprosima::fastdds::dds::DomainParticipant &participant)
    : participant_{participant}, topic_{participant}
{
    ...
    {
        throw std::runtime_error{"Fast DDS could not create the ExplosionEvent Subscriber"};
    }

    reader_ = subscriber_->create_datareader(&topic_.topic(), explosionEventReaderQos(), this);
    if (reader_ == nullptr)
    {
        static_cast<void>(participant_.delete_subscriber(subscriber_));
        subscriber_ = nullptr;
        throw std::runtime_error{"Fast DDS could not create the ExplosionEvent DataReader"};
    }
}

ExplosionEventReader::~ExplosionEventReader() noexcept
{
    if (reader_ != nullptr)
    {
...
void ExplosionEventReader::on_subscription_matched(
...
bool ExplosionEventReader::waitForWriterMatch(const std::chrono::milliseconds timeout)
{
    std::unique_lock lock{matchMutex_};
    return matchChanged_.wait_for(lock, timeout, [this] { return currentMatchCount_ > 0; });
}

bool ExplosionEventReader::waitForData(const std::chrono::milliseconds timeout)
{
    return reader_->wait_for_unread_message(toDdsDuration(timeout));
}

std::expected<std::optional<domain::ExplosionEvent>, ExplosionEventMappingError>
ExplosionEventReader::takeNext()
{
    dds::ExplosionEvent wireEvent;
    eprosima::fastdds::dds::SampleInfo sampleInfo{};
    const auto returnCode = reader_->take_next_sample(&wireEvent, &sampleInfo);
    ...
    {
        return std::optional<domain::ExplosionEvent>{};
    }

    auto mappedEvent = fromWireExplosionEvent(wireEvent);
    if (!mappedEvent.has_value())
    {
        return std::unexpected{mappedEvent.error()};
    }
    return std::optional<domain::ExplosionEvent>{*mappedEvent};
}
